...
\begin{abstract}
The goal of this project is to develop an application that will be able to perform argument mining, by analysing text and to identify the components of arguments. The goal is to be able to spider through the internet and obtain the text from various websites. The aim of this project is that the software created from it will be able to work using the same SADFace format used in many other argumentation-based applications used in Napier University.
\end{abstract}
\pagebreak
...
The structure of arguments is described by Eemeren et al. as a ``constellation of expressed thought contents, called \emph{propositions}'' (Eemeren et al. 2014. p2). Eemeren et al. goes on to suggest that propositions are made up of two connected components. The first component they outline is the ``subject'', which is \emph{what} you are talking about (such as a person, place or item). The second component they outline as the ``predicate'', which is what you are describing the subject \emph{as}. Gilbert presents an example of how this structure can be used in regular speech.\newline
...
[NB: I'd love to have a section here explaining why argumentation research is important, but I haven't found anything regarding it yet, continue looking. Also, aim to talk about the standard argument format we are using (sadface).]
...
\bibitem{eemeren14}
  Eemeren, F.H., Garssen, B., Krabbe, E.C.W., Snoeck Henkemans, A.F., Verheij, B., Wagemans, J.H.M. (2014).
  \textit{Handbook of Argumentation Theory.}
  Dordrecht: Springer.

\bibitem{gilbert17}
  Gilbert, M.A. (2017[maybe 2018?])
